...
Push notifications allow developers to push messages or notifications to users. Keeping interactions with users helps to retain the customers and to improve the user experience. LeanCloud combines Android Push and iOS Push into a single service that you can use directly in your app.

In addition to the iOS and Android SDKs, you can take advantage of the [REST API](#rest-api-push-notifications) to send push notifications.
...
---|---|---
badge|iOS| Number shown on the top right corner of the application icon, which may represent the number of unread messages.
channels||Array of the channels subscribed. The channel name can only contain alphanumerics, underscores (`_`), dashes (`-`), equal signs (`=`), and CJK Unified Ideographs.
deviceProfile||When your application has multiple iOS push certificates or multiple Android mixpush configurations, you can use `deviceProfile` to specify the current certificate or configuration. Its value has to align with the value in [Dashboard > Messaging > Push notifications > Settings](messaging.html?appid={{appid}}#/message/push/conf), otherwise the push will fail. `deviceProfile` must be an empty string or a string that begins with a letter, and only contains letters, numbers, and underscores. `deviceProfile` is a special field which only supports `equals` queries. 
deviceToken|iOS|A unique identifier assigned by Apple for APNs.
...
prod|iOS|The certificate of the environment to use. **dev** represents a development certificate and **prod** represents a production certificate.
...
successes | |The number of successfully pushed devices. "Successfully pushed" means that the notification has been either received by the device or delivered to the upstream push service such as Apple's APNs or Android's FCM.
...
The essence of "push" is to query for all the devices matching some specific conditions and then push a message to these devices.
Because an installation is a fully customizable object, you can easily implement pushes with various complex conditions, such as pushing to users subscribed to certain channels, users located in certain places, etc.

When `devices` has a value of 0, it means there are no devices matching the query conditions provided, and you will need to examine the query conditions.
...
Their difference may be even larger when there are enormous inactive devices.

Changing the attribute `valid` of an installation to `false` will disable push notifications for that device.
...
The targeted devices will still receive the notification pushed to them even if the push history has been cleared.
Refer to the [notification expiration](#expiration-time-and-scheduled-push) section for more information on the expiration of the notification itself.
...
Please refer to the [Push Notifications](leanstorage_guide-js.html#push-notifications) section of the LeanStorage JavaScript Guide.
...
Creating an installation is similar to [creating a normal object](rest_api.html#creating-objects).
